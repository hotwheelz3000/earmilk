...
def main():
    # Load ALL existing posts - these are the source of truth
    try:
    ...
    # Fill in missing artwork for existing posts
    for p in existing_posts:
        if not p.get("artwork") and p.get("artist") and p.get("song"):
            print("  fetching artwork for {} - {}".format(p["artist"], p["song"]))
            p["artwork"] = fetch_artwork(p["artist"], p["song"])

    # Fetch RSS and add only genuinely new posts
