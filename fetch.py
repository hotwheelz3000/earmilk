...
def fetch_url(url, timeout=10):
    req = urllib.request.Request(url, headers={'User-Agent': 'EarmilkFeed/1.0 (github.com/hotwheelz3000/earmilk)'})
        ...
        try:
            ...
            for r in results:
                ...
                if artist.lower() in r_artist or r_artist in artist.lower():
                    ...
                    if art:
                        print('  iTunes hit: ' + r.get('artistName') + ' - ' + r.get('trackName'))
                        return art
            if results:
                art = results[0].get('artworkUrl100', '').replace('100x100bb', '400x400bb')
                if art:
                    print('  iTunes fallback: ' + results[0].get('artistName'))
                    return art
    ...
    try:
        ...
        for rec in recordings:
            ...
            for release in releases:
                ...
                try:
                    art_url = 'https://coverartarchive.org/release/' + release_id + '/front-500'
                    req = urllib.request.Request(art_url, headers={'User-Agent': 'EarmilkFeed/1.0'})
                    with urllib.request.urlopen(req, timeout=6) as resp:
                        if resp.status == 200:
                            print('  MusicBrainz hit: ' + release_id)
                            return resp.url  # follows redirect to actual image
                except Exception:
                    continue
                time.sleep(0.2)
    except Exception as e:
